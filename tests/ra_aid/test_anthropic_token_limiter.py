...
class TestAnthropicTokenLimiter(unittest.TestCase):
    ...
    def test_state_modifier_with_messages(self):
        ...
        with (
            patch(
                "ra_aid.anthropic_token_limiter.create_token_counter_wrapper"
            ) as mock_wrapper,
            patch(
                "ra_aid.anthropic_token_limiter.anthropic_trim_messages"
            ) as mock_trim,
        ):
    ...
    def test_get_model_token_limit_research(self):
        ...
        with (
            patch(
                "ra_aid.anthropic_token_limiter.get_config_repository"
            ) as mock_get_config_repo,
            patch("ra_aid.anthropic_token_limiter.get_model_info") as mock_get_info,
            patch(
                "ra_aid.anthropic_token_limiter.adjust_claude_37_token_limit"
            ) as mock_adjust,
        ):
            mock_get_config_repo.return_value.get.side_effect = (
    ...
    def test_get_model_token_limit_fallback(
        ...
        with patch(
            "ra_aid.anthropic_token_limiter.models_params",
